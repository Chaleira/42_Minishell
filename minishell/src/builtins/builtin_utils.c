...
/*   Updated: 2023/08/08 16:38:09 by rteles-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	builtin_execute(char *print, char **fd, char **length, t_command *command)
{
	if (!print)
		return ;
	*(long *)&fd = !!command->status + 1;
	*(long *)&length = (long)ft_strlen(print);
	write(*(long *)&fd, print, *(long *)&length);
}
